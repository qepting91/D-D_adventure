...
def get_css():
    return """
        ...
        /* --- FIX: REMOVE TOP PADDING --- */
        /* This brings the content up to the top of the phone screen */
        div.block-container {
...
def render_quest_card(icon, title, subtitle, desc, address=None, image_path=None):
    ...
    if image_path:
        ...
        if img_b64:
             html_content.append(f'<img src="data:image/png;base64,{img_b64}" class="quest-image">')

    html_content.append('<hr style="border-color: #b58900; opacity: 0.3; margin: 15px 0;">')
    html_content.append(f'<p>{desc}</p>')
    
    if address:
        encoded_addr = urllib.parse.quote(address)
        waze_url = f"https://waze.com/ul?q={encoded_addr}&navigate=yes"
        html_content.append(f'<a href="{waze_url}" target="_blank" style="text-decoration:none;"><div class="address-box">📍 {address}<br><span style="font-size: 12px; color: #8b0000; text-transform: uppercase;">(Tap to Navigate in Waze)</span></div></a>')
...
# 2. TAVERN
    ...
    nav_buttons(back_target='intro', next_target='armory', next_label=data['nextBtn'])
...
# 3. ARMORY
    ...
    nav_buttons(back_target='tavern', next_target='map', next_label=data['nextBtn'])
